...
class ModelosVehiculosControlador
{
 public async listar (req:Request,res:Response){
    const marcas = await pool.query('SELECT * FROM modelosvehiculos');
    res.json(marcas);
  } 

  public async listarUno (req:Request,res:Response):Promise<any>{
    const { id }= req.params;
    const marcas =  await pool.query('SELECT * FROM modelosvehiculos WHERE codigo=?',[id]);
    if(marcas.length > 0){
  ...
  {
    try {
      const Codigo =await pool.query('SELECT MAX(codigo)+1 AS Codigo FROM modelosvehiculos')
      JSON.stringify(Codigo);
      const codigo = Codigo[0].Codigo;
      console.log(codigo);
      const nombre = req.body.nombre;
      const values={codigo,nombre};
      await pool.query('INSERT INTO modelosvehiculos  SET ?',values);
  ...
  public async actualizar(req:Request, res:Response):Promise<any>
  {
    try {
      ...
      const nombre = req.body.nombre;
      const categoria =  await pool.query('UPDATE modelosvehiculos SET ? WHERE codigo = ?',[ req.body,codigo]);
...
export const modelosVehiculosControlador = new ModelosVehiculosControlador();