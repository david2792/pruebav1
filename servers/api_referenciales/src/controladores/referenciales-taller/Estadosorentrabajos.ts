...
class EstadoTrabajoControlador
{
 public async listar (req:Request,res:Response){
  ...
  {
    try {
      const Codigo =await pool.query('SELECT MAX(codigo)+1 AS Codigo FROM estadosorentrabajos')
      JSON.stringify(Codigo);
      const codigo = Codigo[0].Codigo;
      console.log(codigo);
      const descripcion = req.body.descripcion;
      const values={codigo,descripcion};
      await pool.query('INSERT INTO estadosorentrabajos  SET ?',values);
  ...
  public async actualizar(req:Request, res:Response):Promise<any>
  {
    try {
      ...
      const descripcion = req.body.descripcion;
      const categoria =  await pool.query('UPDATE estadosorentrabajos SET ? WHERE codigo = ?',[ req.body,codigo]);
...
export const estadoTrabajoControlador = new EstadoTrabajoControlador();